...
const SquareRootB: React.FC = () => {
  ...
  const calculateIterations = useCallback((startGuess: number = initialGuess) => {
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <button onClick={animate} disabled={isAnimating} style={{ marginRight: '10px' }}>
          {isAnimating ? 'Animating...' : 'Animate'}
        </button>
        <button onClick={step} disabled={isAnimating || currentIteration === iterations.length - 1} style={{ marginRight: '10px' }}>
          Step
        </button>
        <button onClick={reset} disabled={isAnimating || currentIteration === 0} style={{ marginRight: '10px' }}>
          Reset
        </button>
        <input
          type="number"
      ...
      <div style={{ position: 'relative', width: '100%', height: '400px' }}>
        ...
        <div style={{
          ...
          S/x₀ = {S} / {iterations[currentIteration]?.x.toFixed(4) || 'N/A'} = {iterations[currentIteration]?.complementary.toFixed(4) || 'N/A'}
        </div>
      </div>
      <div style={{ marginTop: '20px' }}>
        <h3>Current Iteration: {currentIteration}</h3>
        <p>x₀ = {iterations[currentIteration]?.x.toFixed(6) || 'N/A'}</p>
        <p>S/x₀ = {S} / {iterations[currentIteration]?.x.toFixed(6) || 'N/A'} = {iterations[currentIteration]?.complementary.toFixed(6) || 'N/A'}</p>
        <p>Next x₀ = (x₀ + S/x₀)/2 = ({iterations[currentIteration]?.x.toFixed(6) || 'N/A'} + {iterations[currentIteration]?.complementary.toFixed(6) || 'N/A'})/2 = {iterations[currentIteration]?.average.toFixed(6) || 'N/A'}</p>
      </div>
      <div style={{ marginTop: '40px' }}>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={currentGraphData}>
            ...
            <Line type="monotone" dataKey="complementary" name="S/x₀" stroke="#82ca9d" />
            <Line type="monotone" dataKey="average" name="Next x₀" stroke="#ffc658" />
            <Line type="monotone" dataKey="sqrtS" name="√S" stroke="#ff7300" strokeDasharray="5 5" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
